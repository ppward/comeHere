...
export default function SignUp(props)
{
  ...
  if (reservedWords.includes(password.toLowerCase())) {
    ...
    violatedRules.push('비밀번호는 최소한 한개 이상의 소문자를 사용해야합니다');
    return false;
  } else if (!/(?=.*[A-Z])/.test(password)) {
    violatedRules.push('비밀번호는 최소한 한개 이상의 대문자를 사용해야합니다');
    return false;
  } else if (!/(?=.*\d)/.test(password)) {
    violatedRules.push('비밀번호는 최소한 한개 이상의 숫자를 사용해야 합니다');
    return false;
  } else if (!/(?=.*[^a-zA-Z0-9])/.test(password)) {
    violatedRules.push('비밀번호는 최소한 한개 이상의 특수문자를 사용해야 합니다');
    return false;
  }else{
    return true;
  }
  ...
  if (violatedRules.length > 0) {
    ...
    setPasswordError('Password must contain at least 6 characters, one uppercase letter, one lowercase letter, and one special character');
    return false;
  } else {
    setPasswordError('');
    return true;
  ...
  const pressDone=()=>{
    ...
    if(checkEmail&&checkNivkname&&checkPassword&&checkConfirm&&checkAge&&checkGender ===true){
      alert("회원가입이 완료되었습니다")
...
//if(email!=null && password!=null&&passwordConfirm!=null && age !=null && gender !=null)
  ...
  return(
    ...
    <TextInput
     ...
     />
     {nicknameError ? <Text style={nicknameError? styles.errorText:styles.checkText}>{nicknameError}</Text> : null}
    <TextInput
     style={styles.input}
     placeholder="email"
     placeholderTextColor="#c1c1c1"
     ...
     />
      {emailError ? <Text style={emailError? styles.errorText:styles.checkText}>{emailError}</Text> : null}
    ...
    <View style ={styles.genderContainer}>
     <TouchableOpacity  style={[ styles.genderButton, gender === '남자'? styles.activeGenderButton: styles.genderButton]}>
        <Text 
      ...
      <TouchableOpacity 
      style={[ styles.genderButton, gender === '여자'? styles.activeGenderButton: styles.genderButton]}
